...
function sendAggregatedFrameIfReady() {
  ...
  if (frameSummaryReady) {
    ...
    if (reporterTMHub.enabled) {
      console.log('> Sending last frame data to Traffic Manager Hub');
      reporters.TMHub.reportCurrentFrame();
    }

    if (_.get(reportersStdOut, '0.enabled')) {
      console.log('> Sending last frame summary data to StdOut');
      reporters.StdOut.reportCurrentFrame();
    }
