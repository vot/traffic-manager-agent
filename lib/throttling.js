...
function shouldBlock(ip) {
  const blockedIps = (config.get('policies.blockedIps') || '').split(',');
  const whitelistedIps = (config.get('policies.whitelistedIps') || '').split(',');
  const throttleLimit = config.get('policies.thresholdBlock');
